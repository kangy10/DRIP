...
! deblurring method implemented in cluster_cwm_deblur.f90. 

! Creator: Yicheng Kang
! Date: July 4, 2017

  Subroutine cluster_cwm_deblur_bandwidth(n, obsImg, nband, bandwidths, zq, sigma, phi0, mean_std_abs, cw, relwt, &
    bandwidth_hat, cv, cv_jump, cv_cty)

    implicit none

    integer :: n, nband, bandwidths(1:nband), bandwidth_hat, cw

    integer :: i, j, k, k1, i1, j1, s, n1clust, n2clust, sl, su, nitem, l1, njump, ncty, &
         ind(1:((2*maxval(bandwidths)+1)**2))

    double precision :: obsImg(0:n, 0:n), zq, sigma, phi0, mean_std_abs, relwt
    ...
    ftilde1(0:(n+2*maxval(bandwidths)), 0:(n+2*maxval(bandwidths))), cv(1:nband), ker1, &
    w00_loo, ahat_loo, cutoff, fhat_loo, cv_jump(1:nband), cv_cty(1:nband), ttemp, zascend_obs(1:((2*maxval(bandwidths)+1)**2)), &
    zmin_obs, zmax_obs
    ...
    do k1 = 1, nband
       ...
       do i = k, n + k
          do j = k, n + k
             ...
             if (e >= asy_thresh) then
                ...
                zmin = zascend(1)
                zmax = zascend(nitem)
                zmin_obs = zascend_obs(ind(1))
                zmax_obs = zascend_obs(ind(nitem))
                ...
                if (ftilde1(i, j) <= cutoff) then
                   ...
                   do i1 = i - k, i + k
                      do j1 = j - k, j + k
                         ...
                         if (ftilde1(i1, j1) <= cutoff .and. abs(ftilde1(i1, j1) - zmin) > epsilon) then

                            ttemp1 = dble(i1 - i) / dble(k)
                            ttemp2 = dble(j1 - j) / dble(k)
                            ttemp = ker(ttemp1, ttemp2) * ker1(abs(ftilde1(i1, j1) - cutoff)/abs(zascend(2) - cutoff))
                   ...
                   !cv(k1) = cv(k1) + (fhat_loo - zmin_obs)**2
                   cv_jump(k1) = cv_jump(k1) + (fhat_loo - zmin_obs)**2
                   ...
                   do i1 = i - k, i + k
                      do j1 = j - k, j + k
                         ...
                         if (ftilde1(i1, j1) > cutoff .and. abs(ftilde1(i1, j1) - zmax) > epsilon) then

                            ttemp1 = dble(i1 - i) / dble(k)
                            ttemp2 = dble(j1 - j) / dble(k)
                            ttemp = ker(ttemp1, ttemp2) * ker1(abs(ftilde1(i1, j1) - cutoff)/abs(zascend(nitem-1) - cutoff))
                            fhat_loo = fhat_loo + z1(i1, j1) * ttemp
                            temp = temp + ttemp
                   ...
                   fhat_loo = fhat_loo / temp
                   !cv(k1) = cv(k1) + (fhat_loo - zmax_obs)**2
                   cv_jump(k1) = cv_jump(k1) + (fhat_loo - zmax_obs)**2
                ...
                cv_cty(k1) = cv_cty(k1) + (ahat_loo - z1(i, j))**2
       ...
       ! if (abs(njump + ncty - (n+1)**2) >= 1) then

       !    print '("Jump points and continuity points do not add up! ")'

       ! end if

       !cv(k1) = cv(k1) / dble((n+1)**2)
       cv_jump(k1) = cv_jump(k1) / dble(njump+1)
       cv_cty(k1) = cv_cty(k1) / dble(ncty+1)
       cv(k1) = relwt * cv_cty(k1) + (1D0 - relwt) * cv_jump(k1)

    end do

    bandwidth_hat = bandwidths(minloc(cv(1:nband), dim=1))

  end Subroutine cluster_cwm_deblur_bandwidth
