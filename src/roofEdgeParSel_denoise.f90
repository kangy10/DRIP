...
subroutine roofEdgeParSel_denoise(n, obsImg, nband, bandwidth, nthresh, thresh, nboot, &
     edge1, dKQ)
  ...
  do i = 1, 7
     ...
     llkbw(i) = i ! Search for the optimal jump-preserving bandwidth by CV.

  end do

  call JP_LLK_CV(n, obsImg, bandw, llkbw, cv)
  ...
  call JP_LLK_fit(n, obsImg, cv_bandwidth, fbhat, resid, sigma)
  ...
  do iband = 1, nband
     ...
     call roofDiff_denoise(n, obsImg, k, diff_orig)
     ...
     do iboot = 1, nboot
        ...
        call roofDiff_denoise(n, bootImg, k, diff_boot)
...
end subroutine roofEdgeParSel_denoise
