! This is a Fortran 90 subroutine to denoise an image using local clustering of center weighted median
! filter output. Bilateral piece-wise constant method is used in the local clustering. Bandwidth is given 
! by users.

  Subroutine cluster_cwm_denoise(n, obsImg, k, zq, sigma, phi0, mean_std_abs, cw, estImg)
    ...
    double precision :: obsImg(0:n, 0:n), sigma, z1(0:(n+2*k), 0:(n+2*k)), &
         ...
         bwr(1:((2*k+1)**2)), cutoff, ttemp, zq, estImg(0:n, 0:n), phi0, &
  ...
  end Subroutine cluster_cwm_denoise
