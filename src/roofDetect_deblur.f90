!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                                                           !
! Fortran source code detects roof/valley edges when blur is!
! present using the edge detector proposed in the paper     !
...
subroutine roofDetect_deblur(n, obsImg, bandwidth, thresh, edge1, edge2)
  ...
  external :: extend,  extend1, ker, ker1
...
!!!!! Read in blurred and noisy observations
  ...
  do i = k, n + k
     do j = k, n + k
        ...
        ! Start to detect roof edge by using LL2K detector. First, every neiborhood
        ! is divided into two halves along the direction perpendicular to (fxx, fxy).

        m2 = sqrt(dhat**2 + fhat**2)
        ...
        do i1 = i - k, i + k
           do j1 = j - k, j + k
              ...
              if ((i1 - i)**2 + (j1 - j)**2 <= k**2) then
                 ...
                 dist = abs(hassperp)/m2
                 aa = ker(ttemp1/ra, ttemp2/ra) * ker1(dist/ra)
  ...
  !! Compare the diff with the threshold.

  do i = 0, n
     do j = 0, n

        if (diff(i, j) > thresh .and. n_edge1(i, j) == 0) then
...
end subroutine roofDetect_deblur
