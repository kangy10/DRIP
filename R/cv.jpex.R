...
print.JPEX_Parameters <- function(x, type = c("cv_scores", "bandwidth", "sigma", "all"), ...) {
...
plot.JPEX_Parameters <- function(x, ...) {
  stopifnot(class(x) == "JPEX_Parameters")
  plot.default(x = x$input, y = x$cv_scores, type = "b", xlab = "Bandwidth", ylab = "CV Scores")
}
...
cv.jpex <- function(image, bandwidths, ncpus = 1) {
  ...
  out1 <- list(LLK = LLK, sigma = sigma, cv_scores = cvs, input = bandwidths, bandwidth = band.min)
