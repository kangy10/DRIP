...
test_that("only accept correct type values", {
  ...
  expect_error(print.JPLLK_Parameters(x = foo, type = "thresh"), "Wrong values for type")
...
test_that("An S3 class JPLLK_Parameters is returned", {
  ...
  expect_no_error(JPLLK_surface(image = matrix(0, 3, 3), bandwidth = 2, plot = TRUE))
})

test_that("only accept proper input", {
  expect_error(JPLLK_surface(image = 1:4, bandwidth = 2), "image data must be a matrix")
  expect_error(JPLLK_surface(image = matrix(0, 3, 4), bandwidth = 2), "image data must be a square matrix")
  expect_error(JPLLK_surface(image = matrix(1, 3, 3), bandwidth = "A"), "bandwidth must be numeric")
})
